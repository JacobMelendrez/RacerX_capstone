let express,engine,bcrypt,sqlite3,open,cookieParser,grantAuthToken,lookUpUserFromAuthToken;_af3‍.x([["dbPromise",()=>dbPromise]]);_af3‍.w("express",[["default",["express"],function(v){express=v}]]);_af3‍.w("express-handlebars",[["engine",["engine"],function(v){engine=v}]]);_af3‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);_af3‍.w("sqlite3",[["default",["sqlite3"],function(v){sqlite3=v}]]);_af3‍.w("sqlite",[["open",["open"],function(v){open=v}]]);_af3‍.w("cookie-parser",[["default",["cookieParser"],function(v){cookieParser=v}]]);_af3‍.w("./authorize",[["grantAuthToken",["grantAuthToken"],function(v){grantAuthToken=v}],["lookUpUserFromAuthToken",["lookUpUserFromAuthToken"],function(v){lookUpUserFromAuthToken=v}]]);
       ...
       const dbPromise = _af3‍.a("open",open)({
    filename: "data.db",
    driver: _af3‍.a("sqlite3",sqlite3).Database,
});_af3‍.j(["dbPromise"]);

const app = _af3‍.a("express",express)();

app.engine("handlebars", _af3‍.a("engine",engine)());
app.set('view engine', 'handlebars');

app.use(_af3‍.a("cookieParser",cookieParser)());
app.use(_af3‍.a("express",express).urlencoded({ extended: false}));
app.use('/static', _af3‍.a("express",express).static(__dirname + '/static'));
...
app.use(async (req, res, next) =>{
    ...
    try {
        const user = await _af3‍.a("lookUpUserFromAuthToken",lookUpUserFromAuthToken)(authToken);
...
app.post('/register', async (req, res)=>{
    ...
    const passwordHash = await _af3‍.a("bcrypt",bcrypt).hash(password, 10);
    try {
        ...
        const token = await _af3‍.a("grantAuthToken",grantAuthToken)(user.id);
...
app.post('/login', async (req, res) =>{
    ...
    try {
        ...
        const passwordMatch = await _af3‍.a("bcrypt",bcrypt).compare(password, existingUser.password);
        if(!passwordMatch){
            throw 'Incorrect login'
        }
        const token = await _af3‍.a("grantAuthToken",grantAuthToken)(existingUser.id);
        res.cookie('authToken', token);
        res.redirect('/');
    } catch (e){
...
app.post('/create_conference', async(req, res) =>{
    ...
    try{
        await db.run('INSERT INTO Events (title, eventDescription) VALUE (?, ?);', event_title, event_description);
